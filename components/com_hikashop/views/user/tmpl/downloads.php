...
<?php
		if($limitNotReached && $periodNotReached) {
			...
			$tooltip = 'data-toggle="hk-tooltip" data-title="'.$downloadFile->file_name.'" data-original-title=""';
			if(in_array(substr($downloadFile->file_path, 0, 1), array('@', '#')) && (int)$downloadFile->file_quantity > 1) {
				...
				$fileHtml = '<a class="'.$css_button.'" '.$tooltip.' href="'.hikashop_completeLink('order&task=download&file_id='.$downloadFile->file_id.'&order_id='.$order_id.$file_pos.$url_itemid).'">'.
					JText::_('DOWNLOAD_NOW').
					'<i class="fas fa-download"></i>'.
				'</a>';
			}
