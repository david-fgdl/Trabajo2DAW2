	...
	foreach($data->cart->products as $item) {
		...
		if(!empty($item->files) && in_array($data->order->order_status,$statusDownload)){
			...
			foreach($item->files as $file){
				...
				$t .= '<a '.$class.'href="'.hikashop_frontendLink('index.php?option=com_hikashop&ctrl=order&task=download&file_id='.$file->file_id.'&order_id='.$data->order_id.$file_pos.$url_itemid).'">'.JText::_('DOWNLOAD_NOW').'</a><br/>';
			}
			$t .= '</p>';
		}
